...
class TimeAccountService {
    async createTimeAccount(userId, hours) {
        const connection = await (0, index_1.connectToDatabase)();
        const [result] = await connection.execute('INSERT INTO time_accounts (user_id, hours) VALUES (?, ?)', [userId, hours]);
    ...
    async getTimeAccountsByUser(userId) {
        const connection = await (0, index_1.connectToDatabase)();
        const [rows] = await connection.execute('SELECT * FROM time_accounts WHERE user_id = ?', [userId]);
        return rows;
    }
    ...
    async getTimeAccountById(id) {
        const connection = await (0, index_1.connectToDatabase)();
        const [rows] = await connection.execute('SELECT * FROM time_accounts WHERE id = ?', [id]);
        const accounts = rows;
        return accounts[0] || null;
    }
    async getRequestById(id) {
        const connection = await (0, index_1.connectToDatabase)();
        const [rows] = await connection.execute('SELECT ta.*, u.username as userEmail FROM time_accounts ta JOIN users u ON ta.user_id = u.id WHERE ta.id = ?', [id]);
        const results = rows;
        return results[0] || null;
    }
    async updateTimeAccountStatus(id, status) {
        const connection = await (0, index_1.connectToDatabase)();
        await connection.execute('UPDATE time_accounts SET status = ? WHERE id = ?', [status, id]);
    }
    async deleteTimeAccount(id) {
        const connection = await (0, index_1.connectToDatabase)();
        const [result] = await connection.execute('DELETE FROM time_accounts WHERE id = ?', [id]);
        const deleteResult = result;
        return deleteResult.affectedRows > 0;
    }
    async approveRequest(accountId) {
        await this.updateTimeAccountStatus(accountId, 'approved');
        return true;
    }
    async rejectRequest(accountId) {
    ...
    async transferHoursToUser(fromUserId, toUserId, hours, reason) {
        try {
            const connection = await (0, index_1.connectToDatabase)();
            // Validierung: Beide User müssen existieren
            const [fromUserRows] = await connection.execute('SELECT id, username FROM users WHERE id = ?', [fromUserId]);
            const [toUserRows] = await connection.execute('SELECT id, username FROM users WHERE id = ?', [toUserId]);
            ...
            const [balanceRows] = await connection.execute('SELECT current_balance FROM user_time_balance WHERE user_id = ?', [fromUserId]);
    ...
    async getUserBalance(userId) {
        try {
            ...
            const [balanceRows] = await connection.execute('SELECT current_balance FROM user_time_balance WHERE user_id = ?', [userId]);
            const balances = balanceRows;
    ...
    async resetTransferHistory(userId) {
        const connection = await (0, index_1.connectToDatabase)();
        try {
            await connection.execute('DELETE FROM time_transfers WHERE from_user_id = ? OR to_user_id = ?', [userId, userId]);
        }
        catch (error) {
...
//# sourceMappingURL=timeaccount.service.js.map
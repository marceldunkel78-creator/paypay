...
class TimeEntryService {
    ...
    async getUserTimeEntries(userId, limit = 50, status) {
        try {
            const db = await (0, db_1.connectToDatabase)();
    ...
    async getUserTimeBalance(userId) {
        try {
            const db = await (0, db_1.connectToDatabase)();
    ...
    async deleteTimeEntry(entryId, userId) {
        try {
            ...
            const [entries] = await db.execute(selectQuery, [entryId, userId]);
            if (entries.length === 0) {
                return false;
            }
            ...
            `;
            const [result] = await db.execute(deleteQuery, [entryId, userId]);
    ...
    async updateUserBalance(userId, hoursChange) {
        try {
            const db = await (0, db_1.connectToDatabase)();
            const query = `
                INSERT INTO user_time_balance (user_id, current_balance)
    ...
    async cleanupOldEntries() {
        try {
            ...
            const selectQuery = `
                ...
                GROUP BY user_id
            `;
            const [oldEntries] = await db.execute(selectQuery);
            // Delete old entries
            const deleteQuery = `
                DELETE FROM time_entries 
                WHERE created_at < DATE_SUB(NOW(), INTERVAL 1 WEEK)
    ...
    async approveTimeEntry(entryId, adminUserId) {
        try {
            const db = await (0, db_1.connectToDatabase)();
            ...
            `;
            const [entries] = await db.execute(selectQuery, [entryId]);
            if (entries.length === 0) {
            ...
            `;
            const [result] = await db.execute(updateQuery, [entryId]);
            if (result.affectedRows > 0) {
    ...
    async rejectTimeEntry(entryId, adminUserId) {
        try {
            const db = await (0, db_1.connectToDatabase)();
            const query = `
                UPDATE time_entries 
                ...
                WHERE id = ? AND status = 'pending'
            `;
            const [result] = await db.execute(query, [entryId]);
    ...
    async updateTimeEntry(entryId, hours, adminUserId) {
        try {
            const db = await (0, db_1.connectToDatabase)();
            ...
            const updateQuery = `
                ...
                WHERE id = ?
            `;
            const [result] = await db.execute(updateQuery, [hours, entryId]);
            if (result.affectedRows > 0) {
                // Balance aktualisieren
    ...
    async updateTimeEntryByMinutes(entryId, inputMinutes, adminUserId) {
        try {
            const db = await (0, db_1.connectToDatabase)();
    ...
    async adminDeleteTimeEntry(entryId) {
        try {
            const db = await (0, db_1.connectToDatabase)();
            ...
            const deleteQuery = `
                ...
                WHERE id = ?
            `;
            const [result] = await db.execute(deleteQuery, [entryId]);
            if (result.affectedRows > 0) {
